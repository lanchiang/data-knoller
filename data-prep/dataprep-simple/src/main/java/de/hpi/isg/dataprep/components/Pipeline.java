...
public class Pipeline implements AbstractPipeline {
	...
	private Pipeline() {
		...
		provenanceRepository = new ProvenanceRepository();
		errorRepository = new ErrorRepository();
		preparations = new LinkedList<>();
	}
	
	private Pipeline(Dataset<Row> rawData) {
		this();
	...
	public Pipeline(DataContext dataContext) {
		...
		rawData = dataContext.getDataFrame();
		datasetName = dataContext.getDialect().getTableName();
	}
...
//        preparation.getAbstractPreparator().buildMetadataSetup();
	...
	public void executePipeline() throws Exception {
		...
		try {
			...
			flatFileWriter.write(getErrorRepository());
			throw e;
		}
	...
	public void initMetadataRepository() {
		FileLoadDialect dialect = dataContext.getDialect();
		...
		StructType structType = rawData.schema();
	...
	public void buildMetadataSetup() {
		...
		preparations.stream().forEachOrdered(preparation -> preparation.getAbstractPreparator().buildMetadataSetup());
	}
	
	@Override
	public void buildColumnCombination() {
		columnCombinations = new HashSet<>();
		StructField[] fields = rawData.schema().fields();
